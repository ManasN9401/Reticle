...
func main() {
	...
	wf, ok := registry.Workflows["startup-launch"]
	if !ok {
		orch.Logger.Error("Workflow startup-launch not found")
		return
	}
	...
	// Add subscriber to save the final plan to disk
	savePlan := func(e events.RuntimeEvent) {
		artifact, ok := e.Payload.(*memory.Artifact)
		if ok && artifact.ID == "launch_plan" {
			if dataStr, isStr := artifact.Data.(string); isStr {
				_ = os.WriteFile("startup_launch_plan.md", []byte(dataStr), 0644)
				fmt.Println("\n✅ Launch plan successfully written to startup_launch_plan.md!")
			}
		}
	}
	orch.Bus.Subscribe(events.EventType("ArtifactStored"), savePlan)
	orch.Bus.Subscribe(events.EventType("ArtifactVersionCreated"), savePlan)

	err := graphEngine.SubmitWorkflow(wf, "exec-startup-001")
	if err != nil {
