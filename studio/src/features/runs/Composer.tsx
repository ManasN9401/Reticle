...
export function Composer() {
  ...
  // Grow with content up to a ceiling: short prompts should not waste space,
  // long ones should stay readable without the panel jumping around.
  useEffect(() => {
  ...
  const submit = async () => {
    ...
    // IDE context is sent pre-formatted so the agents can consume it directly.
    const openFiles = tabs
      .filter((tab) => tab.kind === 'file')
      .map((tab) => tab.subtitle ?? tab.title)
    const ideContext =
