...
| F12 | Partial / External | Configurable image, CPU/memory/process/time limits and explicit CPU/AMD ROCm/NVIDIA CUDA/user profiles replace assumed hardware. Numeric devices receive in-process exclusive leases; native commands use the worker interpreter. A hardware-specific locked training image, cross-process admission and full training/resume validation remain. |
...
| F32 | Partial | Unix process groups, Windows process trees and labelled per-command container cleanup replace immediate-child-only cancellation. A persistent effect ledger recovers uncertain work as interrupted; detached services and ComfyUI still need adapter-specific observation/cancellation implementations. |
| F33 | Repaired retry boundary | CLI/Studio default to three total attempts, bounded to 1–15. SDK internal retries are disabled, including the former architect retry loop. Each concrete attempt has a random identity. Whole-worker retry requires a classified transient provider failure and the SDK's no-effects marker. The durable effect ledger requires reconciliation rather than automatic replay after an uncertain effect. |
...
| F42 | Repaired | Generated definitions/workers are scoped by execution and resolved before global builtins. Environment caches include dependency pins, Python version, OS and architecture. |
...
| F45 | Partial | Lazy, serialized, cancellation-aware provisioning; explicit locked/floating/profile skill policies; exact base pins; Python/platform-aware caches; host interpreter PATH consistency. Floating legacy skills, full transitive locks, immutable image builds and clean-machine install acceptance remain. |
...
| F49 | Partial | Session-spanning batches discard old state; reconnect snapshots restore authoritative node/run states, and replay tolerates out-of-order retained events. Workflow graphs, attempts and terminal state now recover from a local snapshot with in-flight work marked interrupted. Durable event replay/outbox is not implemented. |
...
| F56 | Repaired / External | Runtime/Forge tests, four example builds, Python worker/adapter tests and Studio reducer tests are added; CI configures Go/Python gates on Linux and Windows plus Studio on Linux. Remote CI, signed installers and live services have not been run here. |
...
- Runtime and Forge Go package tests pass, including real child-process output contracts, result-before-dependent dispatch, cancellation-before-submission, approval tampering, attempt idempotency, restart interruption, effect reconciliation, graph mutation admission, device leases, path/control authorization and routing leases.
...
- Fourteen Python unittest cases pass, covering the worker contracts above plus shared-memory prompt visibility, conditional mutations, Windows/AMD profile warnings, credential-literal scanning and the paired memory-quality harness. No LLM or service request is needed for this default suite.
- Six Studio projection tests pass: reconnect snapshots, mixed-session batches, immutable lifecycle transitions, out-of-order replay, fatal-overload handling and persistence-failure interruption.
...
3. Finish the engineering items still marked Partial: transitive dependency/image locks, provider-specific effect reconciliation and remote resource reconciliation. RFC-044 and RFC-045 provide bounded local memory, execution and effect snapshots; a transactional journal/outbox or replicated backend remains future scaling work.
